"""
다국어 언어 분류 실험 - Multi-Seed 버전

여러 LLM 모델로 5개 언어 분류 실험 (Multiple random seeds)
모든 layer에서 hidden states 추출하여 언어 분류 probe 학습

SEEDS = [42, 123, 456, 789, 2024] 로 5회 반복 실행
결과를 mean ± std 형식으로 집계

Usage:
    CUDA_VISIBLE_DEVICES=1 python multilingual_language_classification_multiseed.py
...
DATA_PATH = "/home/dilab05/work_directory/김재성/지티어트랙션_시험/ICLR/Data/multilingual_xnli_5lang.pkl"  # XNLI 데이터셋
OUTPUT_DIR = "./multilingual_experiments_xnli_multiseed"
...
# Hyperparameters (XNLI large dataset)
EXTRACT_BS = 8  # Feature extraction batch size
TRAIN_BS = 128  # Probe training batch size
LEARNING_RATE = 1e-3
WEIGHT_DECAY = 0.01
EPOCHS = 3
MAX_LENGTH = 256
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Feature extraction"):
            ...
            for layer_idx in range(num_layers):
                ...
                all_features[layer_idx].append(last_token_hidden.cpu().numpy())
...
def extract_features_no_save(model, tokenizer, train_dataset, val_dataset, test_dataset):
    """Train/Val/Test의 모든 layer features 추출 (저장 안 함)"""
    print("\n" + "="*70)
...
def run_experiment_for_layer(layer_idx, train_feats, val_feats, test_feats,
    ...
    # Create results list: overall + per language
    results_list = []
...
def run_single_model_experiment(model_name, train_dataset, val_dataset, test_dataset, num_classes, seed):
    """단일 모델에 대한 전체 실험 수행 (특정 seed)"""
    print("\n" + "="*70)
...
def main():
    """메인 함수 - Multi-seed 실험"""
    print("\n" + "="*70)
    print("MULTILINGUAL LANGUAGE CLASSIFICATION - MULTI-SEED EXPERIMENT")
    print("="*70)
    print(f"Models: {len(MODEL_LIST)}")
    for i, model in enumerate(MODEL_LIST, 1):
        print(f"  {i}. {model}")
    print(f"Seeds: {SEEDS} ({len(SEEDS)} runs per model)")
    print(f"Total experiments: {len(MODEL_LIST)} models × {len(SEEDS)} seeds = {len(MODEL_LIST) * len(SEEDS)} runs")
    print(f"Data: {DATA_PATH}")
    print(f"Output: {OUTPUT_DIR}")
    print(f"Device: {DEVICE}")

    # Load data once (shared across all models and seeds)
    ...
    if os.path.exists(RESULTS_FILE):
        ...
        # Get completed (model, seed) pairs
        completed_pairs = set(zip(existing_df['model'], existing_df['seed']))
    else:
        completed_pairs = set()

    initial_count = len(all_results)

    # Loop over seeds first, then models
    for seed in SEEDS:
        for model_idx, model_name in enumerate(MODEL_LIST, 1):
            # Skip if already completed
            if (model_name, seed) in completed_pairs:
                print(f"\n✓ Skipping {model_name} with seed {seed} (already completed)")
                continue

            print("\n" + "#"*70)
            print(f"# SEED {seed} | MODEL {model_idx}/{len(MODEL_LIST)}: {model_name}")
            print("#"*70)
    ...
    print("\n" + "="*70)
    print("SAMPLE AGGREGATED RESULTS (Overall only, first 3 models)")
    print("="*70)
    sample = df_agg[(df_agg['language'] == 'overall')].head(15)
    print(sample[['model', 'layer', 'linear_test_acc_mean_std', 'mlp_test_acc_mean_std', 'mlp_linear_gap_mean_std']].to_string(index=False))
